   ...
   //  BCRec const* pbc = get_velocity_bcrec_device_ptr();
    ...
    amrex::ParallelFor(
        xebox, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            ...
            //std::cout<<"xlo -xhi "<<i<<" "<<j<<" "<<k<<" "<<n<<" "<<Ipx(i-1,j,k,n)<<" "<<Imx(i  ,j,k,n)<<" "<<xlo(i,j,k,n)<<" "<<xhi(i,j,k,n)<<std::endl;
            xlo(i,j,k,n) = lo;
            xhi(i,j,k,n) = hi;
    ...
    amrex::ParallelFor(xbx, [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        ...
        //std::cout<<"qx0  "<<i<<" "<<j<<" "<<k<<" "<<qx(i,j,k)<<" "<<xlo(i,j,k,n)<<" "<<xhi(i,j,k,n)<<" "<<l_dt<<std::endl;
        if (!l_use_forces_in_trans) {
        ...
        qx(i,j,k) = ltm ? 0. : st;
        //std::cout<<"qx  "<<i<<" "<<j<<" "<<k<<" "<<qx(i,j,k)<<" "<<stl<<" "<<sth<<" "<<std::endl;
    });
    //exit(0);
    //
    // Y-Flux
