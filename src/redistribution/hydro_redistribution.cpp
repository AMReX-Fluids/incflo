        ...
        else
        {
            ...
            [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
            {
                          ...
                          (!flag_old(i,j,k).isRegular() || !flag_new(i,j,k).isRegular()) ) )
                {
                    ...
                    if ( i==15 && j==12){
                        Print()<<"1 DELTA DIVU "<<delta_divU
                               <<" "<<Ueb_dot_an
                            //<<" "<<Ueb_dot_an_new
                               <<" "<<delta_vol<<std::endl;
                    }
                    if ( i==16 && j==12){
                        Print()<<"2 DELTA DIVU "<<delta_divU
                               <<" "<<Ueb_dot_an
                            //<<" "<<Ueb_dot_an_new
                               <<" "<<delta_vol<<std::endl;
                    }
                    if ( i==16 && j==11){
                        Print()<<"3 DELTA DIVU "<<delta_divU
                               <<" "<<Ueb_dot_an
                            //<<" "<<Ueb_dot_an_new
                               <<" "<<delta_vol<<std::endl;
...
// FIXME maybe we want to be using flag.isCovered, ect here instead
                ...
                else if ( vfrac_new(i,j,k) == 0.0 && vfrac_old(i,j,k) > 0. )
                {
                    //
                    // Newly covered cell
                    // Give my correction to my neighbor
                    //
                    int nc = 1;
                    if ( itr(i,j,k,0) > 1 ){
                        nc = 3;
                        Print()<<Dim3{i,j,k}<<" is NC and has "<<itr(i,j,k,0)<<" nbs"<<std::endl;
                    }
                    {
                        // No correction for me
                        scratch(i,j,k,n) = U_in(i,j,k,n) + dt * dUdt_in(i,j,k,n);
                        ...
                        if ( Box(scratch).contains(Dim3{i+ioff,j+joff,k+koff}) )
                        {
                            ...
                            {
                                // Use half of Ueb_dot_an and the full delta_vol
                                delta_divU = (Real(0.5)*Ueb_dot_an + delta_vol)*U_in(ii,jj,kk,n);
                            }

                            // This will undo volume scaling that happens later in forming q-hat
                            delta_divU /= vfrac_old(ii,jj,kk);

                            scratch(ii,jj,kk,n) += dt * delta_divU / itr(i,j,k,0);
                            
                            if ( ii==15 && jj==12){
                                Print()<<"31 DELTA DIVU "<<delta_divU
                                       <<" "<<Ueb_dot_an
                                    //<<" "<<Ueb_dot_an_new
                                       <<" "<<delta_vol<<std::endl;
                            }
                            if ( ii==16 && jj==12){
                                Print()<<"32 DELTA DIVU "<<delta_divU
                                       <<" "<<Ueb_dot_an
                                    //<<" "<<Ueb_dot_an_new
                                       <<" "<<delta_vol<<std::endl;
                            }
                            if ( ii==16 && jj==11){
                                Print()<<"33 DELTA DIVU "<<delta_divU
                                       <<" "<<Ueb_dot_an
                                    //<<" "<<Ueb_dot_an_new
                                       <<" "<<delta_vol<<std::endl;
                                // Print()<<"U "<<U_in(i,j,k,n)
