...
#endif
    {
        for (MFIter mfi(a_vel, TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
            ...
            else if (typ == FabType::singlevalued)
            {
                ...
                MOL::EB_PredictVelOnFaces(AMREX_D_DECL(ubx,vbx,wbx),AMREX_D_DECL(u,v,w),vcc,flagarr,
                                          AMREX_D_DECL(fcx,fcy,fcz),ccc, vfrac, a_geom, h_bcrec,
                                          d_bcrec);
            }
            else
#endif
            {
	      MOL::PredictVelOnFaces(AMREX_D_DECL(ubx,vbx,wbx),AMREX_D_DECL(u,v,w),vcc,a_geom,h_bcrec,
                                     d_bcrec);
            }
            }
    }
