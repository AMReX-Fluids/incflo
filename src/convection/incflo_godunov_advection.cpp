                                   ...
                                   Real* p)
{
    ...
    amrex::ParallelFor(
        ...
        {
            Real uad = umac(i,j,k);
            Real fux = (amrex::Math::abs(uad) < 1e-06)? 0. : 1.;
            bool uval = uad >= 0.; 
            auto bc = pbc[n];  
        ...
        {
            Real vad = vmac(i,j,k);
            Real fuy = (amrex::Math::abs(vad) < 1e-06)? 0. : 1.;
            bool vval = vad >= 0.;
            auto bc = pbc[n];
        ...
        {
            Real wad = wmac(i,j,k);
            Real fuz = (amrex::Math::abs(wad) < 1e-06) ? 0. : 1.;
            bool wval = wad >= 0.; 
            auto bc = pbc[n];
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        Real fu = (amrex::Math::abs(wad) < eps) ? 0.0 : 1.0;
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        Real fu = (amrex::Math::abs(vad) < eps) ? 0.0 : 1.0;
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        temp = (amrex::Math::abs(umac(i,j,k)) < 1.e-06) ? 0.5*(stl + sth) : temp;
        qx(i,j,k,n) = temp;
    }); 
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        Real fu = (amrex::Math::abs(uad) < eps) ? 0.0 : 1.0;
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        temp = (amrex::Math::abs(vmac(i,j,k)) < 1e-06) ? 0.5*(stl + sth) : temp; 
        qy(i,j,k,n) = temp;
    });
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        temp = (amrex::Math::abs(wmac(i,j,k)) < 1e-06) ? 0.5*(stl + sth) : temp; 
        qz(i,j,k,n) = temp;
    });
