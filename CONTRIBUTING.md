...
Development generally follows the following ideas:
  ...
  * Bug fixes, questions and contributions of new features are welcome!
       ...
       * All contributions should be done via pull requests.
         ...
         incflo and target the `development` branch. See below for
...
incflo uses [git](https://git-scm.com) for version control. If you
...
git remote add <Jane> git@github.com:<myGithubUsername>/incflo.git
git fetch <Jane>

# Don't push to the main repo. Instead pushes go to your fork.
git remote set-url --push origin git@github.com:<myGithubUsername>/incflo.git
```
...
# Here <Jane> is the name you give to your fork. It does not need to be your github name.
# <myGithubUsername> is your GitHub name.
git remote add <Jane> https://github.com/<myGithubUsername>/incflo.git
git fetch <Jane>

# Don't push to the main repo. Instead pushes go to your fork.
git remote set-url --push origin https://github.com/<myGithubUsername>/incflo.git
```
...
```
# merge incflo main repo's development into current branch
...
and to propose your new feature/improvement/fix to the incflo project.
...
just open a discussion about your plans and ping other incflo developers on it to chime in.
...
Generally speaking, you want to follow the following rules.
  ...
  * Do not merge your branch for PR into your local `development` branch that tracks incflo
    `development` branch.  Otherwise your local `development` branch will diverge from incflo
    `development` branch.

  * Do not commit in your `development` branch that tracks incflo `development` branch.
...
```
After this, the local `development` should be in sync with incflo `development` and your recent
commits have been saved in `new_branch` branch.

## incflo Coding Style Guide

### Code Guidelines

incflo developers should adhere to the following coding guidelines:
...
```
These guidelines should be adhered to in new contributions to incflo, but
